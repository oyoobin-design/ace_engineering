...
export async function GET() {
  try {
    ...
    const xbrlBacklog = backlogFacts
      ? dedupByEnd(
          backlogFacts.filter((f: EdgarFact) => f.form === "10-Q" || f.form === "10-K")
            .sort((a: EdgarFact, b: EdgarFact) => a.end.localeCompare(b.end))
    ...
    const backlog = pressBacklogM != null
      ? [{ period: today, backlogM: pressBacklogM }]
      : xbrlBacklog;
    ...
    return NextResponse.json({
      source: "sec-edgar", entity: "Bloom Energy Corporation (BE)", cik: CIK_PADDED,
      ...
      note: "출처: SEC EDGAR XBRL. Bloom Energy 회계연도 12월 기준.",
    });
  } catch (e) {
