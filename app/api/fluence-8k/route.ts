...
const SUBMISSIONS_URL = "https://data.sec.gov/submissions/CIK0001868941.json";
const CIK = "1868941";
...
type EmbeddingCandidate = {
  ...
  second: TagType;
  secondScore: number;
  lowConfidence: boolean; // 최고 유사도 < 0.25
};
...
async function getEmbeddingCandidates(docTexts: string[]): Promise<EmbeddingCandidate[]> {
  ...
  const anchorTexts = tagNames.map(t => TAG_ANCHORS[t]);
  const allTexts = [...anchorTexts, ...docTexts.map(t => t || "general corporate filing")];
  ...
  return docVecs.map(docVec => {
    const scores = tagNames
      .map((tag, i) => ({ tag, score: cosine(docVec, anchorVecs[i]) }))
      .sort((a, b) => b.score - a.score);

    const first  = scores[0];
    const second = scores[1];
    const lowConfidence = first.score < EMBEDDING_THRESHOLD;
...
아래 ${filings.length}개 Fluence Energy(FLNC) 8-K 공시에 대해 ACE 사업 관점에서 요약과 태그를 JSON으로만 반환해줘.
...
반드시 ${filings.length}개 결과를 반환하라.`.trim();
  ...
  try {
    ...
    return {
      ...
      tags: filings.map((_, i) => {
        const raw = results.find(r => r.index === i)?.tag ?? "";
        return validTags.has(raw) ? (raw as TagType) : "기타";
      }),
    };
  } catch {
    return {
      summaries: filings.map(() => ""),
      tags:      filings.map(() => "기타" as TagType),
    };
  }
}
    ...
    for (let i = 0; i < form.length && filings.length < 5; i++) {
      if (form[i] === "8-K") {
        ...
        filings.push({
          ...
          description:     primaryDocDescription[i] ?? acc,
          accessionNumber: acc,
          docUrl: `https://www.sec.gov/Archives/edgar/data/${CIK}/${accNoDash}/${primaryDocument[i]}`,
        });
      }
    }
